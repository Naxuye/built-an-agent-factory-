...
DEFAULT_TIMEOUT   = int(os.getenv("SANDBOX_TIMEOUT", "120"))     # 秒
...
class SandboxResult:
    ...
    def format_telegram(self) -> str:
        ...
        if self.output:
            preview = self.output[:2000] + "..." if len(self.output) > 500 else self.output
...
async def run_agent(name: str, input_data: Dict[str, Any]) -> SandboxResult:
    ...
    # 2. 注入环境变量
    # project_root 指向 naxuye_project/（skills 所在层），不能到 naxuye-agent/（有项目 main.py）
    ...
    env["PYTHONUTF8"] = "1"
    # agent_path 必须在 project_root 前面，避免同名文件冲突（如 main.py）
    env["PYTHONPATH"] = f"{agent_path}{os.pathsep}{project_root}"

       # 3. 构造启动脚本（通过 stdin 传入 input_data）
    entry_module = entry.replace('.py', '')
    # skills 所在的项目根目录（naxuye_project/）
    runner_script = f"""
...
try:
    ...
    if not isinstance(result, dict):
        result = {{"status": "failed", "error": "run() 返回非 dict"}}
except Exception as _e:
    result = {{"status": "failed", "error": str(_e)}}
