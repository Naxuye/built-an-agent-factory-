...
async def _parse_intent(user_text: str, agent: dict) -> dict:
    ...
    try:
        ...
        system_prompt = (
            "你是参数解析器。你的唯一任务是把用户的自然语言转换成 JSON 参数对象。\n\n"
            f"目标 Agent: {agent_name}\n"
            f"该 Agent 的 run() 函数需要以下参数：\n"
            f"{_json.dumps(input_schema, ensure_ascii=False, indent=2)}\n\n"
            "【规则】：\n"
            "  1. 只输出一个合法 JSON 对象，不要任何其他文字\n"
            "  2. 必须包含上述所有必填字段\n"
            "  3. 从用户的话中提取对应的值\n"
            "  4. 如果用户没有明确指定某个字段，根据上下文合理推断\n"
            "  5. 禁止输出 Markdown 标记\n"
        )
        ...
        logger.info(f"[Broker] 意图解析成功: {parsed}")
        return parsed
...
class Broker:
    ...
    async def _dispatch(self, packet: CommandPacket):
        try:
            ...
            if cmd == Command.HELP:
                ...
                await self._handle_delete(packet)
            else:
    ...
    async def _handle_start(self, packet: CommandPacket):
        ...
        if not name:
            if packet.args:
                name = match_agent_by_keyword(packet.args)
                natural_input = packet.args  # 整段自然语言保留，后面给 LLM 解析
        if not name:
            await self.push(
                "⚠️ 未找到匹配的 Agent。\n"
                "用法：\n"
                "  <code>/start 把星星之火翻译成日语</code>\n"
                "  <code>/start agent_name {\"key\": \"value\"}</code>\n"
        ...
        await self.push(f"🚀 匹配到 <code>{name}</code>，正在启动...")

        # ── 智能输入解析 ──────────────────────────────────────
        input_data = None

        # 1. 如果是自然语言匹配，整段交给 LLM 解析
        if natural_input:
            await self.push("🧠 正在解析输入参数...")
            input_data = await _parse_intent(natural_input, agent)
        # 2. 有 args 但不是自然语言匹配，先试 JSON
        elif packet.args:
            try:
    ...
    async def _handle_delete(self, packet: CommandPacket):
        name = packet.agent_name
        if not name:
            await self.push("⚠️ 请指定 Agent 名称，例如：<code>/delete agent_name</code>")
            return

        agent = get_agent(name)
        if not agent:
            await self.push(f"❌ Agent <code>{name}</code> 未注册，无法删除")
            return

        agent_path = agent.get("path", "")

        # 1. 从注册表注销
        from Nomos.registry import unregister_agent
        success = unregister_agent(name)
        if not success:
            await self.push(f"❌ 注销 <code>{name}</code> 失败")
            return
        ...
        if agent_path and os.path.exists(agent_path):
            try:
                ...
                logger.error(f"[Broker] 删除 Agent 文件失败: {e}")

        msg = f"🗑️ Agent <code>{name}</code> 已删除\n"
        msg += f"  注册表: ✅ 已注销\n"
        msg += f"  文件: {'✅ 已清理' if deleted_files else '⚠️ 文件清理失败或路径不存在'}"
        if agent_path:
            msg += f"\n  路径: <code>{agent_path}</code>"
        await self.push(msg)